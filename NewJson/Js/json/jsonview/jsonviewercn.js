...
Ext.onReady(function () {
	...
	Ext.BLANK_IMAGE_URL = '/Images/jsonview/s.gif';	
	Ext.QuickTips.init();
    ...
    var tree = {
        ...
        bbar: [
	        ...
	        new Ext.form.Label({
	        	...
	        	iconCls: Ext.ux.iconCls.get('/Images/jsonview/arrow_down.png'),
	        	text: 'Next',
	        	handler: function () {
	        		jsonviewer.searchNext();
	        	}
	        }, {
	        	iconCls: Ext.ux.iconCls.get('/Images/jsonview/arrow_up.png'),
	        	text: 'Previous',
	        	handler: function () {
	...
	var jsonviewer = function () {
		...
		return {
			...
			treebuild: function () {
				...
				root.setIcon(Ext.isArray(json) ? '/Images/jsonview/array.gif' : '/Images/jsonview/object.gif');
				this.gridbuild(root);
			},
			...
			json2leaf: function (json) {
				...
				for (var i in json) {
					if (json.hasOwnProperty(i)) {
						if (json[i] === null) {
							ret.push({text: i + ' : null', leaf: true, icon: '/Images/jsonview/red.gif'});
						} else if (typeof json[i] === 'string') {
							ret.push({text: i + ' : "' + json[i] + '"', leaf: true, icon: '/Images/jsonview/blue.gif'});
						} else if (typeof json[i] === 'number') {
							ret.push({text: i + ' : ' + json[i], leaf: true, icon: '/Images/jsonview/green.gif'});
						} else if (typeof json[i] === 'boolean') {
							ret.push({text: i + ' : ' + (json[i] ? 'true' : 'false'), leaf: true, icon: '/Images/jsonview/yellow.gif'});
						} else if (typeof json[i] === 'object') {
							ret.push({text: i, children: this.json2leaf(json[i]), icon: Ext.isArray(json[i]) ? '/Images/jsonview/array.gif' : '/Images/jsonview/object.gif'});
						} else if (typeof json[i] === 'function') {
							ret.push({text: i + ' : function', leaf: true, icon: '/Images/jsonview/red.gif'});
						}
					}
				}
				return ret;
